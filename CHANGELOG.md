...
### 修复
- streaming 模式下回复内容因 `MAX_PREVIEW_CHARS = 2000` 过小导致被截断的问题
  - 卡片内容上限从 2000 提升至 30000 字符（贴近钉钉 AI Card markdown 实际上限）
  - 流式中间更新不再截断，直接发送完整内容
  - `finalize` 时超过上限才截断，并在卡片追加提示；同时自动通过普通消息（分块）补发完整内容
